...
import { call, getInactiveslots } from "redux/axios";
...
function InActive(props) {
  ...
  const fetchSlots = async () => {
    try {
      const res = await call(getInactiveslots({ ts: props.ts }));
      setSlots(res.data);
    } catch (err) {
      console.log(err);
    }
  ...
  const handleKey = (e) => {
    e.key === "Escape" && props.hideInactive();
  };
  ...
  return (
    <div className="inactive-detail-popup">
      <div className="inactive-p-wrap">
        <div className="p-close-btn" onClick={props.hideInactive}>
          <CloseOutlined />
        </div>
        <div className="inactive-p-header">
          <h4>InActive Timesheet Breakup </h4>
        </div>
