...
import { editNode, editModel } from "redux/reducers/Tree";
...
function EditModel() {
  ...
  const [saveRes, setSaveRes] = useState(selected?.save ? true : false);
  const [attrModelOpen, setAttrModelOpen] = useState(false);
  const [attribute, setAttribute] = useState(
    selected?.save || attributes[0] || ""
  );
  ...
  if (!selected?.key) {
    warning("Select node to edit");
    dispatch(editModel(false));
  }
    ...
    <div className="model-container">
      <div className="model-wrap">
        <Button
          className="model-close-btn"
          variant="contained"
          onClick={() => dispatch(editModel(false))}
        >
          {icons.close}
        </Button>
        <div className="model-header">
          <h4>Edit {widgetType} Widget</h4>
        </div>
        <div className="model-body">
          {widgetType === "text" && (
            <div className="model-input-wrap">
              ...
              <FormGroup className="save-checkbox">
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={saveRes}
                      onChange={(e) => setSaveRes(e.target.checked)}
                    />
                  }
